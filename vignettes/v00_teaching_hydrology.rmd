...
vignette: >
  ...
  %\VignetteIndexEntry{Teaching hydrology with airGRteaching}
...
In the `r airGRteaching` GUI, it is possible to test different parameter set of the GR rainfall-runoff models and to estimate the performance of each tested parameter sets in order to perform a manual calibration. A classical way to do so through the `r airGRteaching` GUI is to pick an objective function in the table showing the criteria values on the right, to activate the "Show previous simulations (Qold)" button, and to modify parameter values little by little until the simulation and the criterion are satisfying.
...
You can embed the following code in a loop (except the data processing step using `PrepGR()`). At each iteration you test a new parameter set and compute corresponding the criterion. This way you can find the "best" parameter set.
